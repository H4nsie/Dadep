...
<plugin key="DADEP" name="Day Ahead Dynamic Energy Prices (DADEP)" author="H4nsie" version="1.0.0" wikilink="http://www.domoticz.com/" externallink="https://github.com/H4nsie/DADEP">
    ...
    <params>
        <param field="Username" label="API token ENTSOE " default="5df78702-995f-47f1-b007-b2dc5143f8e8" width="300px" required="true"/>
...
#import lxml   # better to import total module, otherwise it loads twice

#if lxml.etree not in sys.modules:
    
    
##Domoticz.Debug(str(len(lxml))    
...
class BasePlugin:
    ...
    def __init__(self):
        ...
        #self.var = 123
        self.freq = 2 #multiplier for Domoticz.Heartbeat (no need to update frequent)
        ...
        #self.securityToken = '5df78702-995f-47f1-b007-b2dc5143f8e8'
    ...
    def onStart(self):
        ...
        #to be in parameters:
#        self.energiebelasting_stroom = 0.12599
#        self.handlingfee_stroom = 0.0024793
#        self.btw = 0.21
#        self.leverkosten_stroom = 6.25 #per month
        ...
        if (len(Devices) == 0):
            #Domoticz.Device(Name="Stroomprijs", Unit=1, Type=243, Subtype=33, Switchtype=3, Used=1, Options={'ValueQuantity': 'Stroomprijs', 'ValueUnits': 'Ct', 'AddDBLogEntry' : 'true', 'DisableLogAutoUpdate' : 'true'}, DeviceID='DAPEP').Create()
            #Domoticz.Device(Name="Electricity price", Unit=1, Type=243, Subtype=33, Switchtype=3, Used=1, Options={'ValueQuantity': 'Price', 'ValueUnits': 'Ct', 'AddDBLogEntry' : 'true'}, DeviceID='DAPEP_PRICE').Create()
        ...
        #force new data from API at start
    ...
    def getData(self, forcenow=False):
        ...
        if forcenow == False and not current_minute in ["00", "01", "16", "31", "46", "09"]:
            Domoticz.Debug("Not calling API")
            return
        Domoticz.Debug("getData called")
        
        
        #Domoticz.Debug(" DEBUG! "+ self.entsoe_api_url)
        

        
        dayshistory = 2  # 2 days is sufficiant,  but...
        if forcenow:
            dayshistory = 7  # if at startup, please fill history logs.
        ...
        ##Domoticz.Debug("Test value: "+ str(self.dict_hourlyprices["2023-01-14 13:00:00"]))
        #Domoticz.Debug(self.dict_hourlyprices)
        ...
        for i in range(22):   # could be more hours than available in future but struct handles ok.
            ...
            if pointer_hour in self.dict_hourlyprices:
                ...
                if percentage_diff < -(self.colordeviation):
                    ...
                    break
        #UpdateDevice(3, 0, nextgreen + ' ('+str(round(value,0))+'ct)')  #todo: print ct ONLY if found.
...
def get_entsoe_feed(self, periodStart, periodEnd):
    ...
    Domoticz.Debug("Resonse " + str(response.status_code )  ) # 503 temp unanvailable
...
#                Domoticz.Debug(( 'Data for ' + mytimeseries.Period.timeInterval.start.text + ' till ' + mytimeseries.Period.timeInterval.end.text)
            ...
            for myperiod in mytimeseries.Period:
                ...
                for myprices in myperiod.Point:
                    ##print "%s => %s" % (myprices.tag, myprices.text)
                    #Domoticz.Debug( 'here')

                    # get position
                    ##print myprices.position
                    TIMEZONE=0   #todo
                    ...
                    if ( current_hour == this_date_string ):
                        ...
                        UpdateDevice(Unit=1, nValue=0, sValue="-1;"+str(round(total_cost*100,0)), TimedOut=0)      #adjust roud to get more precise price
